...
% Negroni et al model - EC coupling module
...
% 47: INaL h gate
% 48-59: placeholder for INa Markov model
% 60-83: ICaL Markov model - see lines 558-674
% 84-89: myofilament (TSCa; TSCa_star; TSCa_tilde; TS_star; L_p; L_w)
...
Bmax_SR = 19*.9e-3;     % [mM] 
koff_sr = 60e-3;        % [1/ms]    % koff_sr/kon_sr = 0.6e-3 mM
kon_sr = 100;           % [1/mM/ms]
...
KdClCa = (1+(0.704-1)*kPKA_IClCa)*KdClCa; % -29.6% w/ ISO
...
% PKA-dependent myofilament phosphoregulation
...
while (abs(corL)>.00001)
   ...
   FB=Ap*(y(85)+y(87))*(L-y(88))+Aw*y(86)*(L-y(89)); 
   w=FB+Ke*(L-Lz)^5+Le*(L-Lz)-Fm;
   w1=Ap*(y(85)+y(87))+Aw*y(86)+5*Ke*(L-Lz)^4+Le+bet*(Fm+alfa);
   corL=-w/w1;
   L=L+.1*corL; 
   con=con+1;
   if con>200,
...
TS=TSt-y(84)-y(85)-y(86)-y(87);
ER=exp(-RLa*(L-La)^2); 
Yh=Yv*(1-exp(-gama*(L-y(89)-hwr)^2));
if (L-y(89))>hwr;
    Yh=Fh*Yh;
end
fa=f*ER;
ga=Za+Yh;
gd=Yd*exp(-Yc*(L-Lc)); 
ydot(84)=ga*y(86)-fa*y(84)+Yb*TS*y(38)^nc-Zb*y(84);%----------TSCa
ydot(85)=Zr*y(87)*y(38)^nc-Yr*y(85)+Yp*y(86)-Zp*y(85);%-------TSCa*
ydot(86)=Zp*y(85)-Yp*y(86)+fa*y(84)-ga*y(86);%----------------TSCa#
ydot(87)=-gd*y(87)+Yr*y(85)-Zr*y(87)*y(38)^nc;%---------------TS*
ydot(88)=Bp*(L-y(88)-hpr);
ydot(89)=Bw*(L-y(89)-hwr);
if(mechFlag==1),
...
ydot(19) = nc*(ydot(84)+ydot(85)+ydot(86)); % myofilament
ydot(20) = kon_tnchca*y(38)*(Bmax_TnChigh-y(20)-y(21))-koff_tnchca*y(20); % TnCH_ca   [mM/ms]
ydot(21) = kon_tnchmg*Mgi*(Bmax_TnChigh-y(20)-y(21))-koff_tnchmg*y(21);   % TnCH_mg   [mM/ms]
...
ydot(24) = kon_myomg*Mgi*(Bmax_myosin-y(23)-y(24))-koff_myomg*y(24);      % Myosin_mg [mM/ms]
    ...
    %koff_sr=koff_sr*fPKA_PLB; % add CaMKII-dependent phosphoregulation (?)
